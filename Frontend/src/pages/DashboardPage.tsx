...
import {
  Calendar, Mail, Copy, Check, Clock, Users, BarChart2,
  LogOut, Bell, ChevronRight, Inbox, Zap, Plus,
  RefreshCw, ExternalLink, AlertCircle, CheckCircle2,
  Loader2, MessageSquare, TrendingUp, Settings
} from 'lucide-react';
...
const StatusBadge: React.FC<{ status: MeetingStatus }> = ({ status }) => {
  const config = {
    awaiting: { label: 'Awaiting replies', color: 'text-amber bg-amber/10 border-amber/20', icon: <Clock className="w-3 h-3" /> },
    computing: { label: 'Computing overlap', color: 'text-accent-glow bg-accent/10 border-accent/20', icon: <Loader2 className="w-3 h-3 animate-spin" /> },
    booked: { label: 'Booked', color: 'text-teal bg-teal/10 border-teal/20', icon: <CheckCircle2 className="w-3 h-3" /> },
    failed: { label: 'Needs attention', color: 'text-red-400 bg-red-500/10 border-red-500/20', icon: <AlertCircle className="w-3 h-3" /> },
  }[status];

  return (
    <span className={`inline-flex items-center gap-1.5 text-[11px] font-syne font-semibold px-2 py-1 rounded-full border ${config.color}`}>
...
// ── Funnel step ──
const FunnelStep: React.FC<{ label: string; count: number; active?: boolean }> = ({ label, count, active }) => (
  <div className={`flex-1 rounded-xl p-4 border text-center transition-all ${active ? 'bg-accent/5 border-accent/25' : 'bg-panel border-border'}`}>
    <div className={`text-2xl font-display font-bold ${active ? 'text-accent-glow' : 'text-white'}`}>{count}</div>
...
const CopiedToast: React.FC<{ show: boolean }> = ({ show }) => (
  <div className={`fixed bottom-6 right-6 glass rounded-xl px-4 py-3 flex items-center gap-2 text-sm text-teal font-body z-50 transition-all duration-300 ${show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
...
const DashboardPage: React.FC = () => {
  ...
  const copyAddress = () => {
    navigator.clipboard.writeText(calSyncAddress);
    setCopied(true);
    setTimeout(() => setCopied(false), 2500);
  };

  const filteredMeetings = activeTab === 'all' ? meetings : meetings.filter(m => m.status === activeTab);
  ...
  const retryMeeting = (id: string) => {
    setMeetings(prev => prev.map(m => m.id === id ? { ...m, status: 'awaiting' } : m));
  };
  ...
  return (
    <div className="min-h-screen bg-ink flex">
      {/* ── Sidebar ── */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-surface border-r border-border flex flex-col transition-all duration-300 shrink-0 fixed top-0 left-0 h-full z-40`}>
        {/* Logo */}
        ...
        <nav className="flex-1 p-3 space-y-1">
          {[
            { icon: <Inbox className="w-4 h-4" />, label: 'Dashboard', active: true },
            { icon: <Calendar className="w-4 h-4" />, label: 'Meetings' },
            { icon: <Mail className="w-4 h-4" />, label: 'Email threads' },
          ...
          <div className="flex items-center gap-2.5 mb-2">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-xs font-syne font-semibold text-white truncate">{user.name}</p>
                <p className="text-[10px] text-muted font-body truncate">{user.plan === 'free' ? 'Free plan' : 'Pro'}</p>
              </div>
            )}
          </div>
          <button
      ...
      <main className={`flex-1 ${sidebarOpen ? 'ml-64' : 'ml-16'} transition-all duration-300`}>
        ...
        <header className="sticky top-0 z-30 glass border-b border-border px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-syne font-bold text-white">Scheduling Dashboard</h1>
            <p className="text-xs text-muted font-body mt-0.5">All your automated meetings in one place</p>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="relative bg-gradient-to-r from-accent/10 via-accent/5 to-transparent border border-accent/20 rounded-2xl p-5 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-syne font-semibold text-muted uppercase tracking-wider mb-1">Your CalSync address</p>
                <p className="text-lg font-syne font-bold text-white truncate">{calSyncAddress}</p>
                <p className="text-xs text-muted font-body mt-0.5">
                  CC or tag this address in any email thread to trigger automatic scheduling
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Total threads', value: stats.total, icon: <Inbox className="w-4 h-4" />, color: 'text-soft' },
              { label: 'Meetings booked', value: stats.booked, icon: <CheckCircle2 className="w-4 h-4" />, color: 'text-teal' },
              { label: 'Hours saved', value: `${(stats.booked * 0.8).toFixed(1)}h`, icon: <TrendingUp className="w-4 h-4" />, color: 'text-accent-glow' },
              { label: 'In progress', value: stats.computing + stats.awaiting, icon: <Zap className="w-4 h-4" />, color: 'text-amber' },
            ].map((stat, i) => (
              <div key={i} className="bg-panel border border-border rounded-2xl p-5">
                <div className={`${stat.color} mb-3`}>{stat.icon}</div>
                <div className="text-2xl font-display font-bold text-white">{stat.value}</div>
                <div className="text-xs text-muted font-body mt-1">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          {/* ── Funnel ── */}
          <div>
            <h2 className="text-sm font-syne font-semibold text-soft mb-3">Coordination funnel</h2>
            <div className="flex items-center gap-2">
              <FunnelStep label="Awaiting replies" count={stats.awaiting} />
              <ChevronRight className="text-muted shrink-0" />
              <FunnelStep label="Computing overlap" count={stats.computing} active />
              <ChevronRight className="text-muted shrink-0" />
          ...
          <div className="bg-panel border border-border rounded-2xl p-5">
            <h2 className="text-sm font-syne font-semibold text-white mb-4 flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-accent-glow" /> How to trigger CalSync.ai
            </h2>
              ...
              {[
                {
                  ...
                  desc: `Add ${calSyncAddress} to the CC field of any scheduling email`,
                ...
                <div key={step.step} className="bg-surface rounded-xl p-4">
                  ...
                  <h3 className="text-sm font-syne font-semibold text-white mb-1">{step.title}</h3>
                  <p className="text-xs text-muted font-body leading-relaxed mb-3">{step.desc}</p>
                  <div className="bg-ink rounded-lg px-3 py-2 text-[11px] text-teal font-mono whitespace-pre">{step.example}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-syne font-semibold text-white">Email threads</h2>
              <div className="flex items-center gap-1 bg-panel border border-border rounded-xl p-1">
                {(['all', 'awaiting', 'computing', 'booked', 'failed'] as const).map(tab => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-syne font-medium transition-all ${
                      activeTab === tab ? 'bg-accent text-white' : 'text-muted hover:text-soft'
                    }`}
                  >
            ...
            <div className="space-y-2">
              {filteredMeetings.length === 0 && (
                <div className="bg-panel border border-border rounded-2xl p-10 text-center">
                  ...
                  <p className="text-soft text-sm font-body">No threads in this category</p>
                </div>
              )}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${
                        meeting.status === 'booked' ? 'bg-teal'
                          : meeting.status === 'computing' ? 'bg-accent animate-pulse'
                          : meeting.status === 'awaiting' ? 'bg-amber'
                          : 'bg-red-400'
                      }`} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start gap-3 flex-wrap">
                          <h3 className="text-sm font-syne font-semibold text-white">{meeting.subject}</h3>
                        ...
                        <div className="flex items-center gap-4 mt-2">
                          <span className="text-[11px] text-muted font-body flex items-center gap-1">
                            <Users className="w-3 h-3" /> {meeting.participants.length} participant{meeting.participants.length !== 1 ? 's' : ''}
                          </span>
                          <span className="text-[11px] text-muted font-body flex items-center gap-1">
                            <MessageSquare className="w-3 h-3" /> {meeting.threadCount} emails
                          </span>
                          <span className="text-[11px] text-muted font-body">{meeting.createdAt}</span>
                        </div>
                      </div>
                    </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
          ...
          <div className="relative glass rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-syne font-bold text-white mb-1">Start a new meeting thread</h2>
            <p className="text-xs text-muted font-body mb-5">
              Simply CC <span className="text-accent-glow font-medium">{calSyncAddress}</span> in your next scheduling email. Or paste the thread details below.
            </p>
            <div className="space-y-3">
              <input type="text" placeholder="Meeting subject" className="input-field" />
              <input type="email" placeholder="Participant emails (comma separated)" className="input-field" />
              <textarea placeholder="Availability details (optional — e.g. 'I'm free Mon-Wed after 2pm')" className="input-field h-20 resize-none" />
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowAddModal(false)} className="flex-1 py-2.5 rounded-xl border border-border text-muted hover:text-soft text-sm font-body transition-colors">
                Cancel
              </button>
              <button
                onClick={() => {
